...
# Board size
boardWidth = 1.0*0.75 # width of whole whiteboard in meters 
boardHeight = 0.5*0.75 # height of whole whiteboard in meters 
...
# NEW VARIABLES FOR DIFFERENT UP/DOWN SPEEDS
upSpeed = 220
downSpeed = 180
turnUpSpeed = 255
turnDownSpeed = 180
...
while (-1*state_service().state.y < boardWidth - robotWidth): # sweeping left to right until past the scanning area
    if (d > 0): # if the direction is up
        ...
        eraserbot.closed_move_straight(boardHeight - robotReach - state_service().state.x, upSpeed) # move to the top of the board
        eraserbot.closed_tank_pivot(directionRight, downSpeed) # turn to face right
        eraserbot.closed_move_straight(imageWidth, downSpeed) # move over one image width
        eraserbot.closed_tank_pivot(directionDown, downSpeed) # turn to face down
        ...
        while (state_service().state.x - robotReach > imageHeight + robotBack): # while above the bottom of the board
            rospy.sleep(1.0)
            imageCount = take_image(imageCount) # take a picture

            print("Moving down")
            #eraserbot.closed_tank_pivot(directionDown, downSpeed) # correct angle
            eraserbot.closed_move_straight(imageHeight, downSpeed) # move down one image height
            print(state_service().state)

        # this is run after reaching the bottom of the board
        print("Turning left")
        #eraserbot.closed_tank_pivot(directionDown, downSpeed) # correct angle
        eraserbot.closed_move_straight(state_service().state.x - robotReach - 2*robotBack, downSpeed) # move to the bottom of the board
        eraserbot.closed_tank_pivot(directionRight, turnUpSpeed) # turn to face right
        eraserbot.closed_move_straight(imageWidth, upSpeed) # move over one image width
        eraserbot.closed_tank_pivot(directionUp, turnUpSpeed) # turn to face up
